"""
SatMAE + PSANet — chip-based inference for SouthMN segmentation.
"""
...
CHECKPOINT  = '/bigdata/eldawylab/sdas050/MS_Research/SatMAE/output_seg_MN/checkpoint-best.pth'
OUTPUT_DIR  = '/bigdata/eldawylab/sdas050/MS_Research/predictions/satmae_psanet_SouthMN'
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'SouthMN_SatMAE_PSANet_Prediction.tif')

CHIP_SIZE   = 96
STRIDE      = 48
DELTA       = 8
NUM_CLASSES = 14
BATCH_SIZE  = 32   # PSANet uses more memory
...
def main():
    ...
    print(f"\nBuilding SatMAE + PSANet...")
    encoder = models_vit_group_channels.vit_large_patch16(
        patch_size=PATCH_SIZE, img_size=INPUT_SIZE, in_chans=IN_CHANS,
        channel_groups=GROUPED_BANDS, num_classes=0, drop_path_rate=DROP_PATH,
        global_pool=False,
    )
    model = psanet.PSANet(
        encoder=encoder, patch_size=PATCH_SIZE, dropout=DROPOUT,
        classes=NUM_CLASSES, zoom_factor=ZOOM_FACTOR, use_psa=True,
        psa_type=PSA_TYPE, compact=False, shrink_factor=SHRINK_FACTOR,
        mask_h=MASK_H, mask_w=MASK_W, normalization_factor=1.0,
        psa_softmax=True,
    )
    ...
    def flush_batch():
        ...
        with torch.no_grad():
            logits = model(chips_t, is_train=False)
    ...
    for name, (r, c) in tqdm(zip(chip_names, coords), total=len(chip_names), desc="PSANet Inference"):
