"""
visualize_cd_EastNC.py
----------------------
Change detection visualization for EastNC test region.

Binary maps: white=changed, black=unchanged, gray=nodata

Outputs:
    visualizations/cd_EastNC/
        EastNC_CD_GT_colored.png
        EastNC_CD_SpectralGPT_colored.png
        EastNC_CD_Prithvi_colored.png
        legend.png
        geotiffs/  EastNC_CD_GT.tif / SpectralGPT.tif / Prithvi.tif
        crops/highchange/   crop1_T1/T2/GT/SpectralGPT/Prithvi.png  x3
...
def find_informative_crops(gt_ds, preds_ds, n=3, crop_ds=128):
    """SpectralGPT TP high + Prithvi FN high."""
    H, W = gt_ds.shape
    step = crop_ds // 2
    scores = []
    for r in range(0, H-crop_ds, step):
        for c in range(0, W-crop_ds, step):
            ...
            others_fn = sum(((preds_ds[m][r:r+crop_ds, c:c+crop_ds]==0) & changed).sum() for m in other_models)
...
def main():
    ...
    save_png(to_binary_rgb(gt[::SCALE, ::SCALE]),
             f'{OUTPUT_DIR}/EastNC_CD_GT_colored.png')
    save_geotiff_strip(gt, GT_PATH, f'{OUTPUT_DIR}/geotiffs/EastNC_CD_GT.tif')
    ...
    for model_name, pred_path in MODELS:
        ...
        save_png(to_binary_rgb(pred[::SCALE, ::SCALE]),
                 f'{OUTPUT_DIR}/EastNC_CD_{model_name}_colored.png')
        save_geotiff_strip(pred, GT_PATH,
                           f'{OUTPUT_DIR}/geotiffs/EastNC_CD_{model_name}.tif')
