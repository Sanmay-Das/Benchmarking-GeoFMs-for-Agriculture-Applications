"""
visualize_cd_SouthCA.py
-----------------------
Change detection visualization for SouthCA test region.

Binary maps: white=changed, black=unchanged, gray=nodata

Outputs:
    visualizations/cd_SouthCA/
        SouthCA_CD_GT_colored.png
        SouthCA_CD_SpectralGPT_colored.png
        SouthCA_CD_Prithvi_colored.png
        SouthCA_CD_SatMAE_colored.png
        legend.png
        geotiffs/  SouthCA_CD_GT.tif / SpectralGPT.tif / Prithvi.tif / SatMAE.tif
...
GT_PATH = f'{BASE}/predictions/cd_spectralgpt_SouthCA/SouthCA_SpectralGPT_CD_gt.tif'

MODELS = [
    ('Prithvi',
     f'{BASE}/predictions/cd_prithvi_SouthCA/SouthCA_Prithvi_CD_pred.tif'),
]

OUTPUT_DIR = f'{BASE}/visualizations/cd_SouthCA'
SCALE      = 4
STRIP_H    = 256
CROP_SIZE  = 512
...
def find_informative_crops(gt_ds, preds_ds, n=3, crop_ds=128):
    ...
    for r in range(0, H-crop_ds, step):
        for c in range(0, W-crop_ds, step):
            ...
            if valid < crop_ds*crop_ds*0.5: continue
            changed = (gt_p == 1)
            other_models = [m for m in preds_ds if m != 'SpectralGPT']
            sgpt_tp = ((preds_ds['SpectralGPT'][r:r+crop_ds, c:c+crop_ds]==1) & changed).sum() if 'SpectralGPT' in preds_ds else 0
            others_fn = sum(
...
def main():
    ...
    save_png(to_binary_rgb(gt[::SCALE, ::SCALE]),
             f'{OUTPUT_DIR}/SouthCA_CD_GT_colored.png')
    save_geotiff_strip(gt, GT_PATH, f'{OUTPUT_DIR}/geotiffs/SouthCA_CD_GT.tif')

    preds_ds = {}
    for model_name, pred_path in MODELS:
        if not os.path.exists(pred_path):
            print(f"\n{model_name}: SKIPPED (prediction not found: {pred_path})")
            continue
        print(f"\n{model_name}:")
        pred = load_and_align(pred_path, H, W)
        save_png(to_binary_rgb(pred[::SCALE, ::SCALE]),
                 f'{OUTPUT_DIR}/SouthCA_CD_{model_name}_colored.png')
        save_geotiff_strip(pred, GT_PATH,
                           f'{OUTPUT_DIR}/geotiffs/SouthCA_CD_{model_name}.tif')
