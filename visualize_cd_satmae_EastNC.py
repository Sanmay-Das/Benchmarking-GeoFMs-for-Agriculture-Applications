"""
visualize_cd_satmae_EastNC.py
------------------------------
Adds SatMAE to the existing EastNC CD visualization outputs.
SpectralGPT and Prithvi maps are already done — this only generates:
    visualizations/cd_EastNC/
        EastNC_CD_SatMAE_colored.png
        geotiffs/EastNC_CD_SatMAE.tif
...
PRED_PATH  = f'{BASE}/predictions/cd_satmae_EastNC/EastNC_SatMAE_CD_pred.tif'
...
def main():
    os.makedirs(f'{OUTPUT_DIR}/geotiffs',            exist_ok=True)
    os.makedirs(f'{OUTPUT_DIR}/crops/highchange',    exist_ok=True)
    os.makedirs(f'{OUTPUT_DIR}/crops/informative',   exist_ok=True)
    ...
    save_png(to_binary_rgb(pred[::SCALE, ::SCALE]),
             f'{OUTPUT_DIR}/EastNC_CD_SatMAE_colored.png')

    # ── Full-scene GeoTIFF ────────────────────────────────────────────────────
    save_geotiff_strip(pred, GT_PATH,
                       f'{OUTPUT_DIR}/geotiffs/EastNC_CD_SatMAE.tif')
