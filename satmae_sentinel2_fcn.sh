#!/bin/bash
#SBATCH --job-name=satmae_fcn_Iowa
#SBATCH --partition=gpu
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=16
#SBATCH --mem=128G
#SBATCH --time=48:00:00
#SBATCH --output=logs/satmae_fcn_Iowa_%j.out
#SBATCH --error=logs/satmae_fcn_Iowa_%j.err
...
echo "CONFIGURATION"
echo "=========================================="
echo "Decoder:         Prithvi FCNHead"
echo "  Main head:     num_convs=1, channels=256, BN, ReLU, Dropout2d(0.1)"
echo "  Aux  head:     num_convs=2, channels=256, BN, ReLU, Dropout2d(0.1)"
echo "Encoder:         GroupChannels ViT-Large"
echo "Input size:      96x96"
echo "Patch size:      8"
echo "In channels:     18 (6 bands x 3 timesteps)"
echo "Classes:         14 (0=NoData ignored, 1-13=crops)"
echo ""
echo "Optimizer:       SGD (SatMAE paper A.10), momentum=0.9"
echo "Encoder LR:      1e-3  (SatMAE paper A.10)"
echo "Head LR:         1e-2  (SatMAE paper A.10)"
echo "LR scheduler:    Polynomial decay, power=0.9 (SatMAE paper A.10)"
echo "Weight decay:    1e-4  (SatMAE paper A.10)"
echo "Epochs:          100"
echo "Batch size:      8"
echo "Accum iter:      16"
echo "Effective batch: 128   (8 x 16 x 1 GPU)"
echo ""
echo "Loss:            CrossEntropyLoss(ignore_index=0)"
echo "  NoData (class 0) properly masked from training loss"
echo ""
