...
# CONFIGURATION
# ============================================================
CHECKPOINT  = str(seg_checkpoint('satmae', 'NWIA', 'fpn'))
...
def main():
    ...
    start = time.time()
    with torch.no_grad():
        for chips, ys, xs in tqdm(dataloader, desc="SatMAE FPN Inference"):
            chips     = chips.to(device)
            batch_len = chips.size(0)

            logits = model(chips)                        # (B, 14, 96, 96)
