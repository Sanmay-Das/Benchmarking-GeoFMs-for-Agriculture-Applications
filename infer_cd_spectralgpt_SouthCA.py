"""
infer_cd_spectralgpt_SouthCA.py
--------------------------------
Change detection inference for SpectralGPT on SouthCA test chips.
...
CHECKPOINT = str(cd_checkpoint('spectralgpt', 'SouthCA'))
...
def main():
    ...
    with torch.no_grad():
        for _, row in tqdm(df.iterrows(), total=len(df), desc='SpectralGPT CD SouthCA'):
    ...
    pred_path = os.path.join(OUTPUT_DIR, 'SouthCA_SpectralGPT_CD_pred.tif')
    with rasterio.open(pred_path, 'w', **profile) as dst:
        dst.write(pred, 1)
    print(f"Saved prediction: {pred_path}")

    gt_path = os.path.join(OUTPUT_DIR, 'SouthCA_SpectralGPT_CD_gt.tif')
