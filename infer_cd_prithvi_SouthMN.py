"""
infer_cd_prithvi_SouthMN.py
---------------------------
Change detection inference for Prithvi on SouthMN test chips.

Outputs:
    predictions/cd_prithvi_SouthMN/
        SouthMN_Prithvi_CD_pred.tif   -- binary change map (0=unchanged,1=changed,255=nodata)
        SouthMN_Prithvi_CD_gt.tif     -- GT change map
"""
...
BASE       = str(MSR_ROOT)
CHIPS_CSV  = f'{DATA_ROOT}/change_detection_chips/prithvi/SouthMN_chips.csv'
CHECKPOINT = f'{BASE}/prithvi_finetune/ChangeDetection/cd_train_prithvi_MN/best_F1_model.pth'
OUTPUT_DIR = f'{PREDICTIONS}/cd_prithvi_SouthMN'

CHIP_SIZE  = 224

sys.path.insert(0, f'{BASE}/prithvi_finetune/ChangeDetection')

# Normalization stats (Iowa z-score -- same as training via dataset_cd_prithvi.py)
...
def main():
    ...
    skipped = 0
    with torch.no_grad():
        for _, row in tqdm(df.iterrows(), total=len(df), desc='Prithvi CD SouthMN'):
            ...
            try:
                ...
                skipped += 1
                print(f"\n  WARNING: skipping corrupted chip {row['t1']} -- {e}")
                continue
    ...
    if skipped:
        print(f"\nTotal corrupted chips skipped: {skipped}")
    ...
    pred_path = os.path.join(OUTPUT_DIR, 'SouthMN_Prithvi_CD_pred.tif')
    with rasterio.open(pred_path, 'w', **profile) as dst:
        dst.write(pred, 1)
    print(f"Saved prediction: {pred_path}")

    gt_path = os.path.join(OUTPUT_DIR, 'SouthMN_Prithvi_CD_gt.tif')
