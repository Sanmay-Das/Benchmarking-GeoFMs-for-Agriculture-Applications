"""
infer_cd_satmae_SouthMN.py
---------------------------
Change detection inference for SatMAE on SouthMN test chips.

Outputs:
    predictions/cd_satmae_SouthMN/
        SouthMN_SatMAE_CD_pred.tif   — binary change map (0=unchanged,1=changed,255=nodata)
        SouthMN_SatMAE_CD_gt.tif     — GT change map
"""
...
CHIPS_CSV  = f'{BASE}/change_detection_chips/satmae/SouthMN_chips.csv'
CHECKPOINT = f'{BASE}/SatMAE/ChangeDetection/cd_train_satmae_MN/best_F1_model.pth'
OUTPUT_DIR = f'{BASE}/predictions/cd_satmae_SouthMN'
...
def main():
    ...
    with torch.no_grad():
        for _, row in tqdm(df.iterrows(), total=len(df), desc='SatMAE CD SouthMN'):
    ...
    pred_path = os.path.join(OUTPUT_DIR, 'SouthMN_SatMAE_CD_pred.tif')
    with rasterio.open(pred_path, 'w', **profile) as dst:
        dst.write(pred, 1)
    print(f"Saved prediction: {pred_path}")

    gt_path = os.path.join(OUTPUT_DIR, 'SouthMN_SatMAE_CD_gt.tif')
