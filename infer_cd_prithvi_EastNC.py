"""
infer_cd_prithvi_EastNC.py
--------------------------
Change detection inference for Prithvi on EastNC test chips.

Outputs:
    predictions/cd_prithvi_EastNC/
        EastNC_Prithvi_CD_pred.tif   -- binary change map (0=unchanged,1=changed,255=nodata)
        EastNC_Prithvi_CD_gt.tif     -- GT change map
"""
...
BASE       = str(MSR_ROOT)
CHIPS_CSV  = f'{DATA_ROOT}/change_detection_chips/prithvi/EastNC_chips.csv'
CHECKPOINT = f'{BASE}/prithvi_finetune/ChangeDetection/cd_train_prithvi_NC/best_F1_model.pth'
OUTPUT_DIR = f'{PREDICTIONS}/cd_prithvi_EastNC'
...
def main():
    ...
    with torch.no_grad():
        for _, row in tqdm(df.iterrows(), total=len(df), desc='Prithvi CD EastNC'):
    ...
    pred_path = os.path.join(OUTPUT_DIR, 'EastNC_Prithvi_CD_pred.tif')
    with rasterio.open(pred_path, 'w', **profile) as dst:
        dst.write(pred, 1)
    print(f"Saved prediction: {pred_path}")

    gt_path = os.path.join(OUTPUT_DIR, 'EastNC_Prithvi_CD_gt.tif')
