...
# GroupChannels ViT-Large + PSANet
# --------------------------------------------------------
...
class CrossEntropyAuxLoss(nn.Module):
    """CrossEntropy for main + auxiliary PSANet outputs with NoData masking."""
...
def get_args_parser():
    parser = argparse.ArgumentParser('SatMAE Crop Segmentation Fine-tuning', add_help=False)
    ...
    parser.add_argument('--input_size', default=96,  type=int, help='SatMAE native size')
    parser.add_argument('--patch_size', default=8,   type=int, help='SatMAE multi-spectral patch size')
    parser.add_argument('--in_chans',   default=18,  type=int, help='18 = 6 bands x 3 timesteps')
    parser.add_argument('--nb_classes', default=13,  type=int, help='13 crop classes')
    ...
    parser.add_argument('--grouped_bands', type=int, nargs='+', action='append',
                        ...
                        help='Band groups for GroupChannels ViT. '
                             'Pass 3 times: --grouped_bands 0 1 2 3 4 5 '
                             '--grouped_bands 6 7 8 9 10 11 '
                             '--grouped_bands 12 13 14 15 16 17')

    # PSANet
    parser.add_argument('--use_psa',     default=True,  action='store_true')
    ...
    # Loss
    parser.add_argument('--bce_alpha',   default=0.25,  type=float,
                        help='Weight on BCE vs IoU in MultiIoUBCE loss')
    parser.add_argument('--use_ce',      default=False, action='store_true',
                        help='Use CrossEntropy instead of BCE in loss')
    ...
    parser.add_argument('--blr',          default=1e-2, type=float,
                        help='Used only when --lr is not set (scaled by batch size)')
    parser.add_argument('--weight_decay', default=1e-4, type=float,
                        help='Paper A.10: 1e-4')
    ...
    parser.add_argument('--poly_power',   default=0.9,  type=float,
                        help='Power for polynomial LR decay (paper A.10: 0.9)')
    ...
    # Output
    parser.add_argument('--output_dir', default='./output_seg', type=str)
    parser.add_argument('--log_dir',    default='./output_seg', type=str)
...
def main(args):
    ...
    encoder = models_vit_group_channels.__dict__[args.model](
        ...
        global_pool=False,      # MUST be False for segmentation
    )
    ...
    if args.finetune:
        ...
        for k in keys_to_remove:
            if k in checkpoint_model:
                print(f"  Skipping pretrained key: {k}")
                del checkpoint_model[k]

        # Adapt patch_embed weights.
        # Pretrained groups may have a different channel count than fine-tuning
        # (e.g. pretrained 4+4+2 bands, fine-tuning 6+6+6 bands).
        # Fix: average pretrained channels then expand to the target channel count.
        # This is far better than random initialisation.
        ...
        for i in range(num_groups):
            ...
            if w_key in checkpoint_model and w_key in state_dict:
                ckpt_w  = checkpoint_model[w_key]   # [D, C_pre, ph, pw]
                model_w = state_dict[w_key]          # [D, C_ft,  ph, pw]
                if ckpt_w.shape == model_w.shape:
                    print(f"  patch_embed.{i}: exact shape match, loading as-is")
                elif (ckpt_w.shape[0] == model_w.shape[0] and
                      ckpt_w.shape[2:] == model_w.shape[2:]):
                    # Same embed_dim + patch_size, different in_chans
                    C_ft  = model_w.shape[1]
                    avg_w = ckpt_w.mean(dim=1, keepdim=True)        # [D, 1, ph, pw]
                    new_w = avg_w.expand(-1, C_ft, -1, -1).clone()  # [D, C_ft, ph, pw]
                    checkpoint_model[w_key] = new_w
                    print(f"  patch_embed.{i}: adapted "
                          f"{ckpt_w.shape[1]} ch -> {C_ft} ch via channel averaging")
                else:
                    del checkpoint_model[w_key]
                    print(f"  patch_embed.{i}: incompatible shape "
                          f"{ckpt_w.shape} vs {model_w.shape}, skipping")
            if b_key in checkpoint_model and b_key in state_dict:
                if checkpoint_model[b_key].shape != state_dict[b_key].shape:
                    ...
                    print(f"  patch_embed.{i} bias: loading as-is")
        ...
        for k in list(checkpoint_model.keys()):
            if k in state_dict and checkpoint_model[k].shape != state_dict[k].shape:
                print(f"  Shape mismatch, removing: {k} "
                      f"ckpt={checkpoint_model[k].shape} model={state_dict[k].shape}")
                del checkpoint_model[k]
    ...
    # mask_h = 2*6 - 1 = 11
    feat_size = args.input_size // args.patch_size
    shrunk    = (feat_size - 1) // args.shrink_factor + 1
    mask_h    = shrunk
    mask_w    = shrunk
    print(f"PSANet: feat_size={feat_size}, shrunk={shrunk}, mask_h={mask_h}")
    ...
    enc_lr  = args.lr * 0.1   # 1e-3  (paper A.10)
    head_lr = args.lr          # 1e-2  (paper A.10)
    print(f"Encoder LR: {enc_lr:.2e}  |  Head+PSA LR: {head_lr:.2e}")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f"mIoU of the network on val chips: {miou*100:.2f}%")
        ...
        if log_writer is not None:
            ...
            log_writer.add_scalar('seg/val_acc',  test_stats['acc5'], epoch)
