...
class LoadGeospatialAnnotations(object):
    ...
    def __call__(self, results):
        ...
        # Load mask - open_tiff returns (C, H, W)
        gt_semantic_seg = open_tiff(filename)
        
        # Remove channel dimension if present, but keep spatial dims
        if gt_semantic_seg.ndim == 3:
            # (1, H, W) or (C, H, W) -> (H, W)
            gt_semantic_seg = gt_semantic_seg[0]  # Take first channel only
        elif gt_semantic_seg.ndim == 2:
            # Already (H, W) - perfect!
            pass
        else:
            raise ValueError(f"Unexpected mask shape: {gt_semantic_seg.shape}")
