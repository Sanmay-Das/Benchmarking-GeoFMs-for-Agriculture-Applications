...
train_cd_satmae.py
------------------
Training script for SatMAE Change Detection benchmark.

Identical structure to train_cd_spectralgpt.py and train_cd_prithvi.py
with SatMAE-specific dataset and model.
...
Folder structure expected:
    SatMAE/ChangeDetection/
        src/
            model_cd_satmae.py
        train_utils/         <- copy from IEEE_TPAMI_SpectralGPT/downstream_tasks/SegMunich/
        util/                <- copy from SatMAE/util/
        dataset_cd_satmae.py
        train_cd_satmae.py

Run:
    cd SatMAE/ChangeDetection
...
from dataset_cd_satmae import CDDatasetSatMAE
...
def main(args):
    ...
    # datasets
    train_csv = [os.path.join(args.data_root, 'CentIA_chips.csv')]
    val_csv   = [os.path.join(args.data_root, 'EastIA_chips.csv')]
    test_csv  = [os.path.join(args.data_root, 'NWIA_chips.csv')]

    train_dataset = CDDatasetSatMAE(train_csv, training=True)
    val_dataset   = CDDatasetSatMAE(val_csv,   training=False)
    test_dataset  = CDDatasetSatMAE(test_csv,  training=False)
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=CDDatasetSatMAE.collate_fn, drop_last=True,
    )
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=1,
        sampler=val_sampler, num_workers=args.workers,
        collate_fn=CDDatasetSatMAE.collate_fn,
    )
    ...
    # training loop
    best_f1      = 0.0
    results_file = "cd_results_satmae.txt"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('SatMAE Change Detection Training')
    ...
    parser.add_argument('--output-dir', default='./cd_train_satmae')
    parser.add_argument('--device', default='cuda')
    parser.add_argument('-b', '--batch_size', default=16, type=int,
                        help='96x96 chips allow same batch size as SpectralGPT')
