...
class SentinelIndividualImageDataset(SatelliteDataset):
    ...
    # IA Crop Classification
    mean = [1348.2309044530675, 1536.6063529072858, 1320.2527891138272, 1762.1338605620895,
        4265.153824950608, 5722.959857794271, 5780.572889718152, 5950.279629471814,
        3002.3117285252024, 1932.5592919289634]

    std = [191.59013428542548, 219.4006587714216, 239.61121221556175, 292.26895185941294,
       616.6479931292715, 880.829639970807, 890.8668150565022, 894.9639971813718,
       425.0218037710461, 335.87253586950743]
    ...
    def __init__(self,
        ...
        # Check if this is crop classification (has 'label' column) or fMoW (has 'category' column)
        if 'label' in self.df.columns:
            # Crop classification dataset
            self.is_crop_classification = True
            print(f"Loaded crop classification dataset: {len(self.df)} samples")
        else:
            # Original fMoW dataset
            self.is_crop_classification = False
    ...
    def open_image(self, img_path):
        # full_path = os.path.join(self.data_root, img_path) # IA Crop Classification
        with rasterio.open(img_path) as data:
            ...
            # )
            img = data.read()  # (c, h, w)
        assert img.shape[0] == 10, f"Expected 10 bands, got {img.shape[0]} in {img_path}" # IA Crop Classification
    ...
    def __getitem__(self, idx):
        ...
        # Mask bands if specified
        if self.masked_bands is not None:
            images[:, :, self.masked_bands] = np.array(self.mean)[self.masked_bands]
        
        # Get label
        if self.is_crop_classification:
            # Crop classification: direct integer label
            labels = int(selection['label'])
        else:
            # fMoW: category string to index
            labels = self.categories.index(selection['category'])
...
def build_fmow_dataset(is_train: bool, args) -> SatelliteDataset:
    ...
    if args.dataset_type == 'crop_segmentation':
        # For crop segmentation, use train.csv/val.csv/test.csv
        if is_train:
            csv_path = os.path.join(args.data_path, 'train.csv')
        else:
            # Use val.csv for validation during training
            csv_path = os.path.join(args.data_path, 'val.csv')
    else:
        # Original SatMAE datasets
        csv_path = os.path.join(args.train_path if is_train else args.test_path)
    ...
    elif args.dataset_type == 'crop_segmentation':
        mean = CropSegmentationDataset.mean
        std = CropSegmentationDataset.std
        transform = CropSegmentationDataset.build_transform(is_train, args.input_size, mean, std)
        dataset = CropSegmentationDataset(
            csv_path=csv_path,
            transform=transform,
            n_timesteps=getattr(args, 'n_timesteps', 3),
            masked_bands=getattr(args, 'masked_bands', None),
            dropped_bands=getattr(args, 'dropped_bands', None)
        )
        args.nb_classes = 13
...
class CropSegmentationDataset(SatelliteDataset):
    """
    ...
    # Extracted from full 13-band statistics (indices 1, 2, 3, 8, 11, 12)
    mean = [
        1184.3824625,   # B02 (Blue)
        1120.77120066,  # B03 (Green)
        1136.26026392,  # B04 (Red)
        1972.62420416,  # B8A (NIR Narrow)
        1732.16362238,  # B11 (SWIR 1)
        1247.91870117   # B12 (SWIR 2)
    ] * 3  # Repeat 3 times for 3 timesteps = 18 values
    ...
    def __init__(self,
        ...
        Args:
            ...
            n_timesteps: Number of temporal images (default 3)
        ...
        print(f"Loaded CropSegmentationDataset: {len(self.df)} chips")
        print(f"  Timesteps: {n_timesteps}")
        print(f"  Bands per timestep: 6 (B02, B03, B04, B8A, B11, B12)")
        print(f"  Total bands: {self.in_c}")
    
    def __len__(self):
        return len(self.df)
    
    def open_image(self, img_path):
        """Open multi-temporal chip"""
        with rasterio.open(img_path) as data:
            img = data.read()  # (18, 96, 96)
        return img.transpose(1, 2, 0).astype(np.float32)  # (96, 96, 18)
    
    def open_mask(self, img_path):
        """Open corresponding mask"""
        mask_path = str(img_path).replace('.tif', '_mask.tif')
        with rasterio.open(mask_path) as data:
            mask = data.read(1)  # (96, 96)
        return mask.astype(np.int64)
    
    def __getitem__(self, idx):
        """
        Returns:
            img_as_tensor: (18, 96, 96) normalized image
            mask: (96, 96) segmentation labels [0-12]
        """
        selection = self.df.iloc[idx]
        
        # Load image and mask
        images = self.open_image(selection['image_path'])  # (96, 96, 18)
        mask = self.open_mask(selection['image_path'])  # (96, 96)
        
        # Apply masking if specified (not typically used)
        if self.masked_bands is not None:
            images[:, :, self.masked_bands] = np.array(self.mean)[self.masked_bands]
        
        # Apply transform (includes SentinelNormalize)
        img_as_tensor = self.transform(images)  # (18, 96, 96)
        
        # Drop bands if specified (not typically used)
        if self.dropped_bands is not None:
            keep_idxs = [i for i in range(img_as_tensor.shape[0]) if i not in self.dropped_bands]
            img_as_tensor = img_as_tensor[keep_idxs, :, :]
        
        return img_as_tensor, mask
    
    @staticmethod
    def build_transform(is_train, input_size, mean, std):
        """Build transform following SatMAE convention"""
        interpol_mode = transforms.InterpolationMode.BICUBIC
        ...
        if is_train:
            ...
            t.append(SentinelNormalize(mean, std))
            t.append(transforms.ToTensor())
            t.append(transforms.RandomResizedCrop(input_size, scale=(0.2, 1.0), 
