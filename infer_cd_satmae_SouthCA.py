"""
infer_cd_satmae_SouthCA.py
--------------------------
Change detection inference for SatMAE on SouthCA test chips.

Outputs:
    predictions/cd_satmae_SouthCA/
        SouthCA_SatMAE_CD_pred.tif  — binary change map
        SouthCA_SatMAE_CD_gt.tif    — GT change map
"""
...
CHIPS_CSV  = f'{BASE}/change_detection_chips/satmae/SouthCA_chips.csv'
CHECKPOINT = f'{BASE}/SatMAE/ChangeDetection/cd_train_satmae_CA/best_F1_model.pth'
OUTPUT_DIR = f'{BASE}/predictions/cd_satmae_SouthCA'

CHIP_SIZE  = 96

SATMAE_MEAN = np.array([1184.3824625, 1120.77120066, 1136.26026392,
                         1972.62420416, 1732.16362238, 1247.91870117], dtype=np.float32)
SATMAE_STD  = np.array([650.2842772,  712.12507725,  965.23119807,
...
def main():
    ...
    with torch.no_grad():
        for _, row in tqdm(df.iterrows(), total=len(df), desc='SatMAE CD SouthCA'):
    ...
    pred_path = os.path.join(OUTPUT_DIR, 'SouthCA_SatMAE_CD_pred.tif')
    with rasterio.open(pred_path, 'w', **profile) as dst:
        dst.write(pred, 1)
    print(f"Saved prediction: {pred_path}")

    gt_path = os.path.join(OUTPUT_DIR, 'SouthCA_SatMAE_CD_gt.tif')
