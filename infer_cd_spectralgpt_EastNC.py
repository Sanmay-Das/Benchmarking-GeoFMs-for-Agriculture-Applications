"""
infer_cd_spectralgpt_EastNC.py
-------------------------------
Change detection inference for SpectralGPT on EastNC test chips.

Outputs:
    predictions/cd_spectralgpt_EastNC/
        EastNC_SpectralGPT_CD_pred.tif  — binary change map
        EastNC_SpectralGPT_CD_gt.tif    — GT change map
"""
...
CHECKPOINT = f'{BASE}/IEEE_TPAMI_SpectralGPT/downstream_tasks/ChangeDetection/cd_train_spectralgpt_NC/best_F1_model.pth'
OUTPUT_DIR = f'{BASE}/predictions/cd_spectralgpt_EastNC'
...
def main():
    ...
    with torch.no_grad():
        for _, row in tqdm(df.iterrows(), total=len(df), desc='SpectralGPT CD EastNC'):
    ...
    pred_path = os.path.join(OUTPUT_DIR, 'EastNC_SpectralGPT_CD_pred.tif')
    with rasterio.open(pred_path, 'w', **profile) as dst:
        dst.write(pred, 1)
    print(f"Saved prediction: {pred_path}")

    gt_path = os.path.join(OUTPUT_DIR, 'EastNC_SpectralGPT_CD_gt.tif')
