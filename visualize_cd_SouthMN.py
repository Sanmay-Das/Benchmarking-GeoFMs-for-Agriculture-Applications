"""
visualize_cd_SouthMN.py
-----------------------
Change detection visualization for SouthMN test region.

Binary maps: white=changed, black=unchanged, gray=nodata

Outputs:
    visualizations/cd_SouthMN/
        SouthMN_CD_GT_colored.png
        SouthMN_CD_SpectralGPT_colored.png
        SouthMN_CD_Prithvi_colored.png
        SouthMN_CD_SatMAE_colored.png
        legend.png
        geotiffs/  SouthMN_CD_GT.tif / SpectralGPT.tif / Prithvi.tif / SatMAE.tif
...
# -- paths --------------------------------------------------------------------
BASE      = str(MSR_ROOT)
CHIPS_CSV = f'{DATA_ROOT}/change_detection_chips/spectralgpt/SouthMN_chips.csv'
CHIP_SIZE = 128

GT_PATH = f'{PREDICTIONS}/cd_spectralgpt_SouthMN/SouthMN_SpectralGPT_CD_gt.tif'
...
def main():
    ...
    save_png(to_binary_rgb(gt[::SCALE, ::SCALE]),
             f'{OUTPUT_DIR}/SouthMN_CD_GT_colored.png')
    save_geotiff_strip(gt, GT_PATH, f'{OUTPUT_DIR}/geotiffs/SouthMN_CD_GT.tif')
    ...
    for model_name, pred_path in MODELS:
        ...
        save_png(to_binary_rgb(pred[::SCALE, ::SCALE]),
                 f'{OUTPUT_DIR}/SouthMN_CD_{model_name}_colored.png')
        save_geotiff_strip(pred, GT_PATH,
                           f'{OUTPUT_DIR}/geotiffs/SouthMN_CD_{model_name}.tif')
