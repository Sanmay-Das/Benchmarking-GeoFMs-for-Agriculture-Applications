#!/bin/bash
#SBATCH --job-name=infer_cd_EastNC
#SBATCH --partition=gpu
#SBATCH --gres=gpu:a100:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=2:00:00
#SBATCH --output=logs/infer_cd_EastNC_%j.out
#SBATCH --error=logs/infer_cd_EastNC_%j.err
...
mkdir -p "$MSR_OUTPUT_ROOT/logs" "$MSR_OUTPUT_ROOT/predictions/cd_prithvi_EastNC"

echo "==== Prithvi CD Inference -- EastNC ===="
echo "Job:  ${SLURM_JOB_ID:-local}"
echo "Node: $(hostname)"
echo ""

python infer_cd_prithvi_EastNC.py

echo ""
echo "==== Done ===="
ls -lh predictions/cd_prithvi_EastNC/
