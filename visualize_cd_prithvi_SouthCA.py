"""
visualize_cd_prithvi_SouthCA.py
--------------------------------
Change detection visualization for SouthCA test region — Prithvi only.
...
Outputs:
    visualizations/cd_SouthCA/
        ...
        legend.png
        geotiffs/  SouthCA_CD_GT.tif / SouthCA_CD_Prithvi.tif
        crops/highchange/   crop{1,2,3}_{T1,T2,GT,Prithvi}.png
        crops/informative/  crop{1,2,3}_{T1,T2,GT,Prithvi}.png
"""
...
GT_PATH   = f'{BASE}/predictions/cd_prithvi_SouthCA/SouthCA_Prithvi_CD_gt.tif'
...
def find_informative_crops(gt_ds, pred_ds, n=3, crop_ds=128):
    """Crops where Prithvi misses the most change (high FN)."""
    H, W = gt_ds.shape
    step = crop_ds // 2
    scores = []
    for r in range(0, H-crop_ds, step):
...
def main():
    ...
    inf_crops = find_informative_crops(gt_ds, preds_ds['Prithvi'], n=3, crop_ds=crop_ds)
    del gt_ds, preds_ds
    gc.collect()
